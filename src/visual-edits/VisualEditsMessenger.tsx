...
export const CHANNEL = 'page_HOVER_v1' as const;
const VISUAL_EDIT_MODE_KEY = 'page_visual_edit_mode' as const;
const FOCUSED_ELEMENT_KEY = 'page_focused_element' as const;
...
const postMessageDedup = (data: any) => {
  try {
    ...
    if (key === _pageLastMsg) return; // identical – drop
    _pageLastMsg = key;
  } catch {
...
// Helper to parse data-page-id to extract file path, line, and column
const parseid = (
  id: string
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute('data-page-protected', 'true');
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute('data-page-protected');
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the page ID from the element to ensure we're working with the right one
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same page ID
    ...
    const existingStyles = appliedStylesRef.current.get(id) || {};
    appliedStylesRef.current.set(id, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(id);
  ...
  const flushImageSrcChange = () => {
    ...
    const id = imgElement.getAttribute('data-page-id');
    if (!id) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === 'page_STYLE_UPDATE') {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same page ID
        const allMatchingElements = document.querySelectorAll(
          `[data-page-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === 'page_IMAGE_UPDATE') {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-page-id
        const id = element.getAttribute('data-page-id');
        if (id) {
          const parsed = parseid(id);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute('data-page-id');
        ...
        const tagName =
          hit.getAttribute('data-page-name') || hit.tagName.toLowerCase();
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute('data-page-id');
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>('[data-page-id]');
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same page ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-page-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-page-id
